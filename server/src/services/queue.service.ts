...
import { db } from '../db';
import { QueueJob, Notification, EmailOutboxItem, Task, User } from '../types';
...
class NotificationQueueService {
  ...
  public initialize(): void {
    ...
    this.timer = setInterval(() => {
      this.processNextJob();
    }, CONFIG.NOTIFICATION_QUEUE_INTERVAL_MS);
  }
  ...
  public enqueueNotification(params: {
    userId: string;
    task: Task;
    type: 'ASSIGNMENT' | 'STATUS_CHANGE' | 'OVERDUE';
    actor: User;
    ...
    const job: QueueJob = {
      ...
      status: 'QUEUED',
      attempts: 0,
      maxAttempts: CONFIG.NOTIFICATION_MAX_RETRIES,
      createdAt: new Date().toISOString(),
      errorLogs: []
    };

    const savedJob = db.insertQueueJob(job);
    wsService.broadcast('QUEUE_UPDATED', { action: 'JOB_ENQUEUED', job: savedJob });
    return savedJob;
  }
  ...
  private async processNextJob(): Promise<void> {
    ...
    const pendingJobs = db.getPendingQueueJobs();
    ...
    try {
      ...
      wsService.broadcast('QUEUE_UPDATED', { action: 'JOB_PROCESSING', jobId: job.id });

      // Simulated work delay (100ms - 300ms)
      await new Promise(res => setTimeout(res, 200));

      // Check if simulated failures are enabled (for testing retries & DLQ)
      const simulateFailure = db.getSimulateQueueFailures();
      ...
      wsService.sendToUser(job.payload.userId, 'NOTIFICATION_RECEIVED', notification);

      // 2. Stretch Goal: Generate and Dispatch HTML Email
      const targetUser = db.getUserById(job.payload.userId);
      if (targetUser) {
        ...
        db.insertEmailOutbox(emailItem);
        wsService.broadcast('EMAIL_SENT', emailItem);
      }
      ...
      wsService.broadcast('QUEUE_UPDATED', { action: 'JOB_COMPLETED', jobId: job.id });
      ...
      if (newAttempts < job.maxAttempts) {
        ...
        wsService.broadcast('QUEUE_UPDATED', { 
          action: 'JOB_RETRYING', 
          jobId: job.id, 
          attempts: newAttempts, 
          nextRunAt 
        ...
        wsService.broadcast('QUEUE_UPDATED', { action: 'JOB_FAILED_DLQ', jobId: job.id });
      }
    } finally {
  ...
  public retryFailedJobs(): number {
    const failedJobs = db.getQueueJobs(100).filter(j => j.status === 'FAILED');
    let count = 0;
    ...
    wsService.broadcast('QUEUE_UPDATED', { action: 'ALL_RETRIED', count });
    return count;
  }
